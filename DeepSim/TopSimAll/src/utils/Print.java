...
//				continue;
	...
	public static void printByOrderAll(double[][] sim, String outPath, int topk, int testTopK)
			throws IOException {
		// 输出前1000个，防止在mac上内存放不下，其它没出现的默认为0
		BufferedWriter out = new BufferedWriter(new FileWriter(outPath));
		BufferedWriter outsim = new BufferedWriter(new FileWriter(outPath
				+ ".sim.txt"));
		for (int v = 0; v < sim.length; v++) {
...
//		edgeFileToDatabase(MyConfiguration.realdata+"/moreno_crime_crime.txt", "edges_test");
	}
