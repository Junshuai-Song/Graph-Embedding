...
/**
 * 对于每个顶点的情况，我们分别使用singleWalk和DoubleWalk处理，Single高label为1，否则为-1。
 * @author luoxiongcai,Alan
 *
 */

public class ProduceLabels {
	...
	public ProduceLabels(Graph g, int sample, int step) {
		this.STEP = step;
		this.SAMPLE = sample;
		this.g = g;
...
//				System.out.println("");
		...
		this.out.append(cnt1 + " " + cnt2 + " " + cnt3 + "\n");	//增加其他特征
		...
		for(int i=0;i<fileNum;i++){
			...
			for(int step: steps){
				for (int sample_ : samples){	// 这里step加上之后，路径还要进一步调整
					...
					ProduceLabels pro = new ProduceLabels(g,sample,step);
...
//						Eval.precision(goldPath, outPath, prePath,k);
		...
		System.out.println("对于生成的文件，在使用的时候需要重排序.");
	}
