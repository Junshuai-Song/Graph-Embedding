...
public class TopSim_Basic {
	...
	public TopSim_Basic(Graph g, int sample, int step) {
		this.STEP = step;
		this.SAMPLE = sample;
		this.g = g;
...
//				System.out.println("cur = " + cur.cur + "  degree = " + degree + "  sample = " + cur.sample);
				
//				int flag=0;
//				for(int j=0;j<=pathLen;j++){
//					for(int k=j+1;k<=pathLen;k++){
//						if(path[j].cur == path[k].cur){
//							flag=1; break;
//						}
//					}
//					if(flag==1) break;
...
//					queue.add(path);
//					queue.remove();	// 最后再删除
//					continue;	//不满足firstMeet
//				}
				if(degree==0)
					System.out.println(cur.sample + " " + degree);
				if(degree !=0 && cur.sample >= degree){
					
					// 最起码每条都要送去一条边(newSample条)
//					int[] edges = new int[degree+1];
					List<Integer> edges;
...
//					int pre = -2;
//					if(pathLen == 2){
//						pre = path[pathLen-1].cur;	// 当前点前一个点
//					}
...
//						if(nextCur.cur == pre){
//							continue;	// 不满足firstMeet，直接略过
//						}
//						int flag=0;
//						for(int k=1;k<=pathLen;k++){
//							if(path[k].cur == nextCur.cur){
//								flag=1; break;
//							}
////							for(int k=j+1;k<=pathLen;k++){
////								if(path[j].cur == path[k].cur){
...
////							if(flag==1) break;
//						}
//						if(flag==1) continue;	//不满足firstMeet
